...
Email signup and organization/plan selection are human steps. Setup accepts a management token from the official account page. Ordinary PATs have user-account privileges and configurable expiry, rather than inherent project scoping. Current endpoint docs also list fine-grained permissions; use those where available and sufficient. An app's anon/service-role keys and its database password are separate credentials. [Management authorization](https://supabase.com/docs/reference/api/introduction), [signup](https://supabase.com/dashboard/sign-up).
