...
3. At **Connect Supabase**, sign up/sign in and choose a dedicated organization, stopping before manual project creation. Open the access-token page, choose **Organization** resource access and select only that organization. Apply the six entries in the on-screen permission table; leave everything else at None. The No access preset alone cannot work. Paste the **Supabase management token** and choose **Save Supabase token & read accounts**. Do not use a project API key or database password. [Detailed settings and expiry](account-guide.md).
...
5. After Setup reserves your website address, follow **Google Cloud**: create/select one dedicated Google project, enable YouTube Data API v3, configure the scopes/audience, create a **Web application OAuth client**, and copy the exact origin and `/api/auth` callback shown. Enter Project ID, OAuth client ID and OAuth client secret on this same step. This version's form requires a published external audience or an eligible internal Workspace audience; **External · still Testing** does not enable Continue. Google's audience/verification settings are separate from Windows signing.
