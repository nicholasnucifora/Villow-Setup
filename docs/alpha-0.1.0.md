...
6. Click **Prepare my database**. Setup reads the Session pooler address from Supabase and uses the stored password plus the bundled Supabase root certificate. If access needs attention, use the visible instructions for **Supabase → project → Connect → Session pooler**, port 5432. Save its exact host/user before retrying; leave the password blank unless you changed it. Do not select Transaction pooler or recreate the project. Continue through configuration and deployment. At **Make it yours**, open the hosted app, sign in with the intended Google owner and return to check the installation.
