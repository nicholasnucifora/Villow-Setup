...
1. Install and launch the Alpha. Confirm the unsigned-alpha notice is visible. At **Prepare your accounts**, follow the three service pages. Sign into Vercel and prepare a Supabase organization, stopping before manual project creation; Setup creates the dedicated Vercel/Supabase projects. Create/select one dedicated Google Cloud project and keep its dashboard bookmark.
2. Click **Check the official release**. It must show **Villow 0.1.0 — Authenticated release**. Choose an instance name and the Google email that should own it, read the dedicated-resource/cost confirmation and select **Start my setup**.
3. At **Connect your accounts**, use the linked provider pages to create a **Vercel access token** and **Supabase management token**. Paste each into its named field inside the Alpha, then select **Save tokens & read my accounts**. Do not use a Supabase project API key or database password as the management token.
...
5. After Setup reserves your website address, follow **Connect Google**: enable YouTube Data API v3, configure the listed scopes/audience, create a **Web application OAuth client**, and copy the exact origin and `/api/auth` callback shown. Enter Project ID, OAuth client ID and OAuth client secret in the Alpha. This version's form requires a published external audience or an eligible internal Workspace audience; **External · still Testing** does not enable Continue. Google's audience/verification settings are separate from Windows signing.
