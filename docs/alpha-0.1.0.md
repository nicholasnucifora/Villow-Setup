...
5. After Setup reserves your website address, follow **Google Cloud**: create/select one dedicated project, enable YouTube Data API v3, choose **External** and keep **Testing**. Under **Audience**, scroll below **OAuth user cap**, click **Test users → + Add users**, enter your Google owner email, save and check it appears in the table. Follow the inline screenshot and acknowledge the seven-day Google access limit. In Data Access, use **Copy scope list** and the two inline scope references. Create a **Web application OAuth client** with the exact origin and `/api/auth` callback shown. The client-created screenshot shows which two copy buttons to use. Enter Project ID, client ID and secret, confirm the configuration and save. Publishing is not part of this walkthrough.
6. Click **Prepare my database**. Setup reads the Session pooler address from Supabase and uses the stored password plus the bundled Supabase root certificate. If access needs attention, follow **Supabase → project → Connect → Direct (Connection string) → Connection Method → Session pooler**, port 5432. The inline picture highlights Connect. Ignore the Framework package-install instructions; expand View parameters beneath the connection string if needed. Save the exact host/user before retrying; leave the password blank unless you changed it. Do not select Transaction pooler or recreate the project. Continue through configuration and deployment. At **Make it yours**, open the hosted app, sign in with the intended Google owner and return to check the installation.
