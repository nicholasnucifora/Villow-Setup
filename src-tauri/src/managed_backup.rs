//! Temporary recovery protection for the single supported Alpha repair. Paths
...
pub const KEY: &str = "repair_backup_key";
...
pub fn path(store: &Store, id: &str) -> Result<PathBuf> {
    ...
    let root = store
        ...
        .canonicalize()
        .map_err(|_| Error::BackupStorage)?;
    let directory = root.join("repair-backups");
...
pub fn prepare(
    ...
    let saved = store.load()?.ok_or(Error::RepairRefused)?;
    if saved.id != s.id
        || saved.operation_id != s.operation_id
        || saved.release_digest != s.release_digest
        || saved.installed_repair.is_some()
        || saved.step != Step::Health
        || saved.read_only
    ...
    if metadata(&path)?.is_some() {
        ...
        if previous.format != 1
            ...
            || checkpoint.installed_repair.is_some()
        {
...
pub fn cleanup(store: &Store, vault: &dyn Vault, s: &mut Installation) -> Result<()> {
    let Some(p) = &s.installed_repair else {
        return Ok(());
    };
    ...
    if saved.read_only
        ...
        || saved.id != s.id
        || saved.release_digest != s.release_digest
        || saved.installed_repair != s.installed_repair
    {
        return Err(Error::RepairRefused);
    }
    let path = path(store, &s.id)?;
